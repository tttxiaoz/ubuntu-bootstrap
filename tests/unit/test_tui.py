"""tui 的降级分支测试（强制走纯文本路径，不依赖 rich/questionary）。"""
...
def test_packages_default():
    assert tui._packages(Config({})) == ["rich", "questionary"]


def test_packages_from_cfg():
    assert tui._packages(Config({"ui": {"packages": ["rich"]}})) == ["rich"]
