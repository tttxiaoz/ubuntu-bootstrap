...
from bootstrap.tasks import ssh as ssh_module
from bootstrap.tasks.ssh import SshTask


def _cfg(pass_auth=True, root_login=False):
    return Config({"ssh": {"password_auth": pass_auth, "permit_root_login": root_login}})
...
def test_config_ok(monkeypatch):
    ...
    assert t._config_ok(_cfg(True, False)) is True
...
def test_config_not_ok_root_login(monkeypatch):
    ...
    assert t._config_ok(_cfg(True, True)) is False
...
def _patch_ss(monkeypatch, stdout):
    monkeypatch.setattr(ssh_module.psys, "command_exists", lambda n: n == "ss")
    monkeypatch.setattr(ssh_module.psys, "run_cmd", lambda *a, **k: _Proc(stdout))
    return SshTask()
...
def test_service_active_falls_back_to_port(monkeypatch):
    monkeypatch.setattr(ssh_module.psys, "command_exists", lambda n: n == "ss")
    monkeypatch.setattr(ssh_module.psys, "run_cmd",
