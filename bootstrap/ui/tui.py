"""终端原语：rich（彩色展示）+ questionary（交互），失败逐级降级到 ANSI 纯文本。

依赖探测采用 lazy import；首次运行若缺失会通过 pip 自动安装
（PEP 668 限制用 --break-system-packages 兜底）。任何一步失败都降级，
不影响初始化功能。
"""
...
def _packages(cfg) -> list:
    return list(cfg.get("ui.packages") or ["rich", "questionary"])


def _index_url(cfg) -> str:
    return cfg.get("ui.pip_index_url") or ""
...
def _pip_install(cfg, log=None) -> bool:
    ...
    first = psys.run_cmd(base + pkgs, check=False, capture=True)
    if first.returncode == 0:
        return True
    err = (first.stderr or "") + (first.stdout or "")
    if "externally-managed" in err or "break-system-packages" in err:
        retry = psys.run_cmd(base + ["--break-system-packages"] + pkgs,
...
def ensure_deps(cfg, log=None) -> tuple[bool, bool]:
    ...
    if rich_available() and questionary_available():
        return True, True
    msg = ("未检测到交互 UI 依赖（rich/questionary），正在通过 pip 安装"
           "（网络不通约 1 分钟后自动跳过）...")
    if log:
        log(msg)
    else:
